...
1. **CORS**: The API endpoints support cross-origin requests for Electron apps (see CORS Configuration below).
...
5. **File Size Limits**: JSON files max 10MB, Images max 50MB.
6. **PHP Requirements**: The `fileinfo` extension must be enabled in `php.ini` for file uploads to work.

---

## CORS Configuration

For the API to work with Electron apps or other external clients, CORS must be properly configured.

### Laravel Configuration

The CORS settings are in `config/cors.php`:
...
|---------|-------|-------------|
| `paths` | `['api/*']` | CORS applies to all `/api/*` routes |
| `allowed_origins` | `['*']` | Allows requests from any origin (Electron apps, browsers, etc.) |
| `allowed_methods` | `['*']` | Allows all HTTP methods (GET, POST, etc.) |
| `allowed_headers` | `['*']` | Allows all request headers |

### Security Note

The default configuration allows all origins (`*`). For production environments, consider restricting `allowed_origins` to specific domains:

```php
'allowed_origins' => [
    'http://localhost:*',           // Local development
    'file://*',                     // Electron apps
    'app://your-electron-app',      // Custom Electron protocol
],
```

### Verifying CORS

Test CORS is working by making a request from your Electron app:

```typescript
// This should work without CORS errors
const response = await fetch('http://your-server:8000/api/uploaded-annotations');
const data = await response.json();
console.log(data);
```

If you see CORS errors in the browser/Electron console, check:
1. The `config/cors.php` file exists
2. Laravel's CORS middleware is active (it is by default in Laravel 11+)
